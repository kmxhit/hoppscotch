...
interface CapabilityCheck {
  matcher: (req: HoppRESTRequest | HoppRESTResponseOriginalRequest) => InspectionMatch | null
...
export class RequestInspectorService extends Service implements Inspector {
  ...
  private readonly inspectionChecks: CapabilityCheck[] = [
    ...
    {
      matcher: (req) => req.body.contentType === "application/octet-stream" ? {} : null,
  ...
  private validateCapability(
    ...
    return capabilities.value[requires.type].has(requires.name)
      ? [createInspection(match)]
      : []
  }
